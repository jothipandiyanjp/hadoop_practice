...
public class SampleYahooFinanceTask{
	  ...
	  private static HashMap<String, YahooFinance> originalData = new HashMap<String, YahooFinance>();
	...
	public static void process(URL url) {
		...
		while (true) {
			try {
				...
				list.parallelStream()
				.forEach(stockDetailByCompany-> originalData.put(stockDetailByCompany.getSymbol(),stockDetailByCompany));
