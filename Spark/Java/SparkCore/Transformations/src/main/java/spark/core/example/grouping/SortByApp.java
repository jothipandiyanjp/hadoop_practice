...
public class SortByApp implements Serializable{

	private static final long serialVersionUID = 1L;
	private static final Logger log = Logger.getLogger("SortByApp");
	...
	//	JavaRDD<String> lines= ctx.textFile("hdfs://192.168.1.193:9000/jo/Sample.csv");
		...
		JavaRDD<Tuple2<Integer, CsvToJava>> groupedData = 
				listOfData.sortBy(f -> f._2.getName(), false,2 );
		
		List<Tuple2<Integer, CsvToJava>> result = groupedData.collect();
		for(int i=0;i<result.size();i++)
			log.info("result -> "+result.get(i));
	}
		
	public static void main(String[] args) throws InterruptedException {
		SortByApp num = new SortByApp();
		num.countLineLength();
	}
